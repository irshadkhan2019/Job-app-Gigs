...
import { IRatingTypes, IReviewMessageDetails, ISellerDocument, ISellerGig } from '@irshadkhan2019/job-app-shared';
...
//   document data is in _source in ES : gigs {total,hits}
...
// gets all inactive gigs for seller
...
//   mongodb created _id and ES will throw error if _id is given so ,
  if (createdGig) {
    ...
    // Finally add the gig in gigs index/like collection in mongodb.
    await addDataToIndex('gigs', `${createdGig._id}`, data);
  }
  return createdGig;
};

// remove from mongo,ES and decrement totalGIgsCount for a seller by 1.
const deleteGig = async (gigId: string, sellerId: string): Promise<void> => {
    // delete from mongo
  await GigModel.deleteOne({ _id: gigId }).exec();
//   decrement totalGIgsCount for a seller by 1.
  await publishDirectMessage(
    gigChannel,
    'jobber-seller-update',
    'user-seller',
...
// set a particular gig as active or inactive
...
//     five: { value: { type: Number, default: 0 }, count: { type: Number, default: 0 }},
  const gig = await GigModel.findOneAndUpdate(
    ...
    { new: true, upsert: true } //upsert means if property doesn't exist then add them. 
  ).exec();
  if (gig) {
    // update in ES
...
// Used in consumer of gigs itself no called directly.
